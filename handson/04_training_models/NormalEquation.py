...
def plot_softmax(iris):
    from sklearn.linear_model import LogisticRegression
    X = iris["data"][:, (2, 3)]  # petal length & width
    y = iris["target"]  # unchanged targets
    ...
    plt.contourf(x0, x1, zz, cmap=custom_cmap)  # displays bg colors
    contour = plt.contour(x0, x1, zz1, cmap=plt.cm.brg)  # displays curved lines
    plt.clabel(contour, inline=1, fontsize=12)  # adds labels to lines
    plt.xlabel("Petal length", fontsize=14)
    plt.ylabel("Petal width", fontsize=14)
    plt.legend(loc="center left", fontsize=14)
    plt.axis([0, 7, 0, 3.5])

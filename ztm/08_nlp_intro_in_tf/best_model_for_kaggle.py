...
def load_train_test_data(train_path=TRAIN_PATH, test_path=TEST_PATH):
    ...
    return train_df["text"], train_df["target"], test_df["text"]


def fit_USE(X_train, y_train, X_test, num_epochs=5, X_val=None, y_val=None):
...
def run():
    ...
    fit_USE(train_sentences, train_labels, test_sentences, num_epochs=5, X_val=val_sentences, y_val=val_labels)
